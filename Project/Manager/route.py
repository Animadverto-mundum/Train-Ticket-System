...
def route_add():
    ...
    if request.method == 'POST':
        ...
        # 没有错误，对数据库进行更新
        if error is None:
            new_line = Line(line_name=route_name, departure_station=departure_station, arrival_station=arrival_station, line_distance=line_distance)
...
def route_edit():
    ...
    route = Line.query.filter(Line.line_ID == route_id).first()  # 查找当前修改的用户
    if request.method == 'POST':
        # 获取表单数据
        route_name = request.form['route_name']
        departure_station = request.form['departure']
        ...
        # 没有错误，对数据库进行更新
        if error is None:
            new_line = Line.query.filter(Line.line_name == route.line_name).update({
